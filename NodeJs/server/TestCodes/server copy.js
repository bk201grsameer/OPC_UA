...
const { Client, UserTokenType } = require('node-opcua');
...
const connectToOPCUA = async () => {
  try {
    opcuaClient = new Client({ endpointMustExist: false });
    await opcuaClient.connect(url);

    rl.question('[+] ENTER YOUR USERNAME : ', (user) => {
      rl.question('[+] ENTER YOUR PASSWORD : ', async (pass) => {
        await opcuaClient.createSession({ type: UserTokenType.UserName, userName: user, password: pass });
